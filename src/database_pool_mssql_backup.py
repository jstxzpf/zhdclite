...
class ConnectionPool:
    def __init__(self, config_path="config/mssql.json", pool_name="MSSQL", max_connections=10, timeout=30):
...
def get_mssql_connection_pool():
    """获取MSSQL数据库连接池的单例"""
    global _pool
    if _pool is None:
        with _pool_lock:
            if _pool is None:
                _pool = ConnectionPool('config/mssql.json', pool_name="MSSQL")
    return _pool
